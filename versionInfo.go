	...
	for _, v := range manifest.Versions {
		...
		go func() {
			...
			if err != nil {
				...
				Error.Printf("[!] An error has occurred while downloading %s\n", id)
				fmt.Println(err)
			...
			if err != nil {
				...
				Error.Printf("[!] An error has occurred while parsing %s\n", id)
			...
			if !okSha {
				Warn.Printf("[!] Version %s has no SHA1. Skipping...\n", id)
				return
			}
...
func getVersionInfos() ([]VersionInfo, error) {
	...
	if err != nil {
		Info.Println("[+] Version manifests are corrupted. Dowloading them again...")
		return updateVersionInfos()
	}
